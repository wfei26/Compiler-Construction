...
void dump(int blev, FILE *f)
{
	...
	for(int i = 0; i < ITABSIZE; i++) 
	{
		if(id_table[i]) 
		{
			struct id_entry *ptr = id_table[i];
			while(ptr) 
			{
				if(ptr->i_blevel >= blev)
				{
					fprintf(f, "%s\t", ptr->i_name);
					fprintf(f, "%i\t", ptr->i_blevel);
					fprintf(f, "%i\t", ptr->i_type);
					fprintf(f, "%i\n", ptr->i_defined);
				}
				ptr = ptr->i_link;
			}
...
void leaveblock()
{
	...
	for(int i = 0; i < ITABSIZE; i++) 
	{
		if(id_table[i]) 
		{
			...
			while(ptr) 
			{
				if(ptr->i_blevel == level)
				{
					id_table[i] = ptr->i_link;	
					free(ptr);
				}
				ptr = ptr->i_link;
			}
